...
# 1.2 定义一个OutputSchema
class MyAgentOutput(TypedDict):

    llm_answer:str

# 1.3 定义一个InputSchema
class MyAgentInput(TypedDict):
...
# 传入input_schema和output_schema，使图输入和输出都做一个过滤
builder = StateGraph(state_schema = MyAgentState,output_schema=MyAgentOutput,input_schema=MyAgentInput)
# 3.1 添加节点
...
res = graph.invoke({"query":"什么是langgraph","rag_search":"xxxxx"}) # 得到的res，就是状态的最终结果
# 当前得到的res里面，仅包含MyAgentOutput定义的字段
print(res)
